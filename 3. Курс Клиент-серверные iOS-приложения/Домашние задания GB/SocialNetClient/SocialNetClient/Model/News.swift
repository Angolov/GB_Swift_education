...
class VKUserNews: Object, Decodable {
    ...
    var photosNames = List<UserPhotoSize>()
    ...
    convenience required init(from decoder: Decoder) throws {
        ...
        self.dateInString = DateFormatterHelper.formatter.string(from: self.date)
        ...
        for item in attachments {
            if item.type == "photo" {
                self.photosNames.append(objectsIn: item.sizes)
            }
        }
    }
}

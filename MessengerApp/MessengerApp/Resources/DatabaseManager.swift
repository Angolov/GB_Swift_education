...
extension DatabaseManager {
    ...
    public func sendMessage(to conversation: String, otherUserEmail: String, name: String, newMessage: Message, completion: @escaping (Bool) -> Void) {
        ...
        // update recepien latest message
            ...
            strongSelf.database.child("\(conversation)/messages").setValue(currentMessages) { error, _ in
                ...
                strongSelf.database.child("\(currentEmail)/conversations").observeSingleEvent(of: .value) { snapshot in
                    ...
                    if var currentUserConversations = snapshot.value as? [[String: Any]] {
                        ...
                        databaseEntryConverstion = [
                            newConversationData
                        ]
                    }
                    
                    strongSelf.database.child("\(currentEmail)/conversations").setValue(databaseEntryConverstion) { error, _ in
                        guard error == nil else {
                            completion(false)
                            return
                        }
                        ...
                        strongSelf.database.child("\(otherUserEmail)/conversations").observeSingleEvent(of: .value) { snapshot in
                            ...
                            if var otherUserConversations = snapshot.value as? [[String: Any]] {
                                ...
                                if var targetConversation = targetConversation {
                                    ...
                                    databaseEntryConverstion = otherUserConversations
                                }
                                else {
                                    let newConversationData: [String: Any] = [
                                        "id": conversation,
                            ...
                            strongSelf.database.child("\(otherUserEmail)/conversations").setValue(databaseEntryConverstion) { error, _ in
                                guard error == nil else {
                                    completion(false)
                                    return
                                }
    ...
    func conversationExists(with targetRecepientEmail: String,

...
//        services.fetchVKData(ofType: .friends)
    ...
    private func operationsHW() {
        
        let queue = OperationQueue()
        
        let friendsVKRequestOperation = FriendsVKRequestOperation()
        let friendsDataParsingOperation = FriendsDataParsingOperation()
        let friendsDataSaveToStorageOperation = FriendsDataSaveToStorageOperation()
        
        friendsDataParsingOperation.addDependency(friendsVKRequestOperation)
        friendsDataSaveToStorageOperation.addDependency(friendsDataParsingOperation)
        
        let operations = [friendsVKRequestOperation, friendsDataParsingOperation, friendsDataSaveToStorageOperation]
